...
浏览器测试配置指定系统 Google Chrome（`channel: chrome`），不是仅安装 Playwright 默认 Chromium。当前开发机最近结果：62 项单测、18 项浏览器测试通过；常规测试会跳过 4 个长测／录屏案例。Vite 的共享 Three.js 分块约 538 kB，有 500 kB 体积提示，不是构建失败。
